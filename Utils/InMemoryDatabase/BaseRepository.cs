...
namespace InMemoryDatabase
{
    ...
    public class BaseRepository<TEntity> : BaseRepository, IBaseRepository<TEntity> where TEntity : class
    {
        ...
        public async Task<IEnumerable<TEntity>> FindAsync(Action<IRangedBatchSelectSqlSqlStatementOptionsOptionsBuilder<TEntity>>? statementOptions = null)
        {
            if (_unitOfWork.Transaction != null)
            {
                statementOptions += x => x.AttachToTransaction(_unitOfWork.Transaction);
            }
            return await _unitOfWork.Connection.FindAsync<TEntity>(statementOptions);
        }

        public async Task<IEnumerable<TEntity>> WhereAsync(FormattableString whereClause, object parameters)
        {
            if (_unitOfWork.Transaction != null)
            {
        ...
        {
            return await _unitOfWork.Connection.UpdateAsync(entityToUpdate, AttachTransaction);
        }
